...
unsigned int nTargetSpacing = 2 * 60; // 2 minute blocks
...
const string strMessageMagic = "BrexitCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	std::string sSeed = pindexBest->GetBlockHash().ToString().substr(7,7);
    const char* cSeed = sSeed.c_str();
    long nSeed = hex2long(cSeed);
    int nRandom = generateMTRandom(nSeed, 1000000);
	int64_t nBonusMultiplier = 1;
	
    if(nRandom <= 5000 * 2) // 10% chance
        nBonusMultiplier = 2;
    if(nRandom <= 2500 * 2) // 5% chance
        nBonusMultiplier = 3;
    if(nRandom <= 1500 * 2) // ~3% chance
        nBonusMultiplier = 5;
    if(nRandom <= 500 * 2) // 1% chance
        nBonusMultiplier = 10;
    if(nRandom <= 250 * 2) // 0.5% chance
        nBonusMultiplier = 20;
    if(nRandom <= 125 * 2) // 0.25% chance
        nBonusMultiplier = 50;
    if(nRandom <= 5 * 2) // 0.01% chance
        nBonusMultiplier = 100;
			
	if (pindexBest->nTime <= 1468440000)
		return nSubsidy * nBonusMultiplier * 10 + nFees;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BrexitCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
		...
		// block.GetHash() == 00000e3d7bc1c16a33c7209e1c5f5f455b44692c20f7cbdb0b4ad7ddeb502982
		// block.hashMerkleRoot == c52b9da1d0c9f87955eee4a30ab05976f20999a4a471713fdb0910f073035a1f
		// block.nTime = 1466193310 
		// block.nNonce = 350709 
	
        const char* pszTimestamp = "June 24, 2016 - England voted strongly for Brexit, by 53.4% to 46.6%, as did Wales, with Leave getting 52.5% of the vote and Remain 47.5%.";
        CTransaction txNew;
        txNew.nTime = 1466193310;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc52b9da1d0c9f87955eee4a30ab05976f20999a4a471713fdb0910f073035a1f"));
...
unsigned char pchMessageStart[4] = { 0x12, 0xa3, 0xc7, 0xc9 };
