	...
	stakeForCharityDialog(0)
{
    updateStyle();
    resize(1000, 740);
    setWindowTitle(tr("BrexitCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BrexitCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BrexitCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to the BrexitCoin network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    if(count < nTotalBlocks)
    {
        ...
        if (strStatusBarWarnings.isEmpty())
        {
            ...
            progressBarLabel->setVisible(false);
            progressBar->setFormat(tr("Synchronizing with the BrexitCoin network:   %n block(s) remaining", "", nRemainingBlocks));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BrexitCoin address or malformed URI parameters."));
    }
...
{
    if (!fUseBrexitCoinTheme)
        return;

    QString qssPath = QString::fromStdString( GetDataDir().string() ) + "/BrexitCoin.qss";
